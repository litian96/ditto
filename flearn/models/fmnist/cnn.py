...
class Model(object):
    ...
    def get_kl_gradients(self, data, output2):
        with self.graph.as_default():
            kl_grads = self.sess.run(self.kl_grads,
                                        feed_dict={self.features: data['x'], self.labels: data['y'], self.output2: output2})

        return kl_grads


    def get_data_gradients(self, data):
        data_grads_op = tf.gradients(self.loss, self.features)
        with self.graph.as_default():
            data_grads = self.sess.run(data_grads_op, feed_dict={self.features: data[0], self.labels: data[1]})
        return data_grads
    ...
    def get_softmax(self, data):
        with self.graph.as_default():
            soft_max = self.sess.run(self.soft_max, feed_dict={self.features: data['x'], self.labels: data['y']})
        return soft_max
