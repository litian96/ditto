...
class Server(BaseFedarated):
    ...
    def train(self):
        ...
        for idx, c in enumerate(self.clients):
            if idx in corrupt_id:
                ...
                if self.dataset == 'celeba':
                    ...
                    c.train_data['y'][i] = np.random.randint(0, 62, len(c.train_data['y']))
        ...
        # local finetuning based on EWC
        after_test_accu = []
        test_samples = []
        for idx, c in enumerate(self.clients):
            c.set_params(self.latest_model)
            weights = copy.deepcopy(self.latest_model)
            weights = [np.zeros_like(layer) for layer in weights]
            for _ in range(4):
                data_batch = next(batches[c])
                c.set_params(self.latest_model)  # optimal global model
                _, grads, _ = c.solve_sgd(data_batch)
                for layer in range(len(grads[1])):
                    weights[layer] += np.square(grads[1][layer])
            for layer in range(len(weights)):
                weights[layer] = weights[layer] / 4
            ...
            for _ in range(max(int(self.finetune_iters * c.train_samples / self.batch_size), self.finetune_iters)):
                ...
                for j in range(len(grads[1])):
                   eff_grad = grads[1][j] + self.lam * np.multiply(weights[j], (local_model[j] - self.latest_model[j]))
