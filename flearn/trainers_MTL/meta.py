...
class Server(BaseFedarated):
    def __init__(self, params, learner, dataset):
        print('Using meta-learning to Train')
            ...
            if idx in corrupt_id:
                ...
                if self.dataset == 'celeba':
                    ...
                    for i in range(len(c.train_data['y'])):
                        c.train_data['y'][i] = np.random.randint(0, 62)

            if self.dataset == 'celeba':
                batches[c] = gen_batch_celeba(c.train_data, self.batch_size, self.num_rounds * self.local_iters * 3 + 300)
            else:
                batches[c] = gen_batch(c.train_data, self.batch_size, self.num_rounds * self.local_iters * 3 + 300)
        ...
        for i in range(self.num_rounds + 1):
            ...
            for idx in indices:
                ...
                for _ in range(self.local_iters):
                    ...
                    data_batch = next(batches[c])
                    delta = 0.001
                    dummy_model1 = [(u + delta * v) for u, v in zip(local_model, grads)]
                    dummy_model2 = [(u - delta * v) for u, v in zip(local_model, grads)]
                    ...
                    for layer in range(len(grads)):
                        local_model[layer] = local_model[layer] - beta * grads[layer] + self.learning_rate * beta * correction[layer]

                diff = [u - v for (u, v) in zip(local_model, weights_before)]
